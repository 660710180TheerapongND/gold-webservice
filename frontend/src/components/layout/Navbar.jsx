...
export default function Navbar() {
  ...
  // ฟังก์ชันเช็กว่า Link ไหนกำลัง Active อยู่
  ...
  return (
    <nav style={{ 
      ...
      <Link to="/" style={{ 
        ...
        <span style={{ 
          fontFamily: C.serif, fontSize: '20px', fontWeight: 600, 
          fontStyle: 'italic', color: '#fff', letterSpacing: '0.05em' 
        }}>
          Gold <span style={{ fontFamily: C.sans, fontWeight: 800, fontStyle: 'normal', fontSize: '15px', color: C.goldLt }}>TRACKER</span>
        </span>
      </Link>
      ...
      <div style={{ 
        ...
        <div style={{ marginLeft: '1rem' }}>
          {apiKey ? (
            /* ลบ Link Profile ออก เหลือเพียงปุ่ม Logout */
            <button 
              onClick={() => { 
                localStorage.removeItem('gt_api_key'); 
                navigate('/'); 
                window.location.reload(); // บังคับรีเฟรชเพื่อให้สถานะ Navbar อัปเดต
              }}
              style={{ 
                background: 'none', 
