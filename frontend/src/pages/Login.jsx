...
const C = {
  bg: '#0A0906', surface: '#110F0B', gold: '#C49A3C', goldLt: '#F0C96E',
  goldDk: '#7A5C1A', smoke: 'rgba(240, 201, 110, 0.06)', border: 'rgba(196, 154, 60, 0.18)',
  muted: 'rgba(255,255,255,0.28)', text: '#F5EDD6', serif: "'Cormorant Garamond', serif",
...
function FloatInput({ label, name, type = 'text', value, onChange }) {
  ...
  return (
    <div style={{ position: 'relative', marginBottom: '18px' }}>
      <label style={{ position: 'absolute', left: '12px', top: active ? '0px' : '50%', padding: active ? '0 5px' : '0', transform: 'translateY(-50%)', backgroundColor: active ? (focused ? '#16140F' : C.surface) : 'transparent', fontSize: active ? '10px' : '14px', fontFamily: C.sans, fontWeight: 600, letterSpacing: active ? '0.12em' : '0.02em', textTransform: active ? 'uppercase' : 'none', color: focused ? C.goldLt : C.muted, transition: 'all 0.22s cubic-bezier(0.4,0,0.2,1)', pointerEvents: 'none', zIndex: 2 }}>{label}</label>
      <input name={name} type={inputType} value={value} onChange={onChange} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} required style={{ width: '100%', background: focused ? 'rgba(196,154,60,0.05)' : C.smoke, border: `1px solid ${focused ? C.gold : C.border}`, borderRadius: '10px', color: C.text, fontFamily: C.sans, fontSize: '15px', padding: active ? '22px 48px 10px 16px' : '18px 48px 18px 16px', outline: 'none', transition: 'all 0.22s ease', boxSizing: 'border-box', caretColor: C.goldLt }} />
      {isPass && (
        <button type="button" onClick={() => setVisible(v => !v)} style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: focused ? C.gold : C.muted, fontSize: '11px', fontFamily: C.mono, padding: '4px', zIndex: 3 }}>
...
export default function Login() {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: C.sans, padding: '24px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ background: C.surface, borderRadius: '20px', border: `1px solid ${C.border}`, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ padding: '28px 24px' }}>
              <form onSubmit={handleSubmit}>
                ...
                <button type="submit" disabled={loading} className="submit-btn" style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', background: `linear-gradient(130deg, ${C.goldDk} 0%, ${C.gold} 50%, ${C.goldLt} 100%)`, color: '#0A0906', fontWeight: 700, fontSize: '13px', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: loading ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                  {loading ? "Signing in..." : "Sign In"} <ArrowRight size={16} />
                </button>
                <Divider label="OR" />
                <button type="button" onClick={() => navigate('/signup')} style={{ width: '100%', padding: '13px', borderRadius: '12px', border: `1px solid ${C.border}`, background: 'transparent', color: C.muted, fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>
                  สร้างบัญชีใหม่ฟรี
                </button>
              </form>
            </div>
